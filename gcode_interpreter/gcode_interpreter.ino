...
void gcodeProcessCommand(const char *cmd)
{
  ...
  if (cmd_type == 'G')
  {
    if (cmd_id == 0 || cmd_id == 1) /* G0 y G1: Move. */
      gcodeMove(cmd_argc, (const char **)cmd_argv);
    else if (cmd_id == 90) /* G90: Set to Absolute Positioning. */
      g_absPos = true;
    else if (cmd_id == 91) /* G91: Set to Relative Positioning. */
      g_absPos = false;
  }
  // /Custom Debug commands for moving the servos directly
  else if (cmd_type == 'D')
  {
    if (cmd_id == 1)
      dCodeMove(cmd_argc, (const char **)cmd_argv);
  }
  // Not implmented, ignore
...
void gcodeMove(u32 cmd_argc, const char **cmd_argv)
{
  ...
  double r_shoulder = 0.0, r_elbow = 0.0;
...
void dCodeMove(u32 cmd_argc, const char **cmd_argv)
{
  if (cmd_argc <= 1 || !cmd_argv)
    return;
  double shoulder = 0.0, elbow = 0.0;
  ...
  {
    char arg_type = cmd_argv[i][0];
    if (arg_type != 'S' && arg_type != 'E')
      continue;
    double *val = (arg_type == 'S' ? &shoulder : &elbow);
    *val = strtod(cmd_argv[i] + 1, NULL);
  }
  /* Verificar si los ángulos generados están dentro del rango. */
  /* Mover brazo robótico. */

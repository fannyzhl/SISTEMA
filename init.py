...
def arduinoSendGcode(gcode_path):
    ...
    try:
        ...
        traceback.print_exc()
        messagebox.showerror('Error', '¡Error al leer líneas del archivo G-code generado!', parent=g_tkRoot)
        return False
    ...
    for line in lines:
        ...
        if not g_arduinoObj.send(line.encode('utf-8')):
            messagebox.showerror('Error', '¡Error de envío al Arduino!', parent=g_tkRoot)
            return False
...
def imageTracingThreadFunc(name, last_name, png_path, image):
    ...
    success = ((convertPngToSvg(image, svg_path, MAX_WIDTH_MM, MAX_HEIGHT_MM, OFFSET_X_MM, OFFSEY_Y_MM) == True) and (convertSvgToGcode(svg_path, gcode_path) == True))
    ...
    if success:
        ...
        messagebox.showerror('Error', '¡El proceso de conversión PNG -> SVG -> G-code falló!', parent=g_tkRoot)
    ...
    try:
        ...
        traceback.print_exc()
        messagebox.showerror('Error', "¡No se pudo guardar el registro de conversión en base de datos!", parent=g_tkRoot)
        return
    
    messagebox.showinfo('Información', "¡Conversión realizada exitosamente!\nSVG almacenado en \"{}\".\nG-code almacenado en \"{}\".".format(svg_path, gcode_path), parent=g_tkRoot)
...
def uiToggleElements(enable):
    ...
    g_tkNameText['state'] = g_tkLastNameText['state'] = g_tkOpenPngButton['state'] = obj_state
...
def uiOpenPng():
    ...
    name = re.sub(r"\s+", " ", g_tkNameText.get('1.0', tk.END).strip(), flags = re.MULTILINE)
    #print(bytes(name, encoding='utf8'))
    
    last_name = re.sub(r"\s+", " ", g_tkLastNameText.get('1.0', tk.END).strip(), flags = re.MULTILINE)
    #print(bytes(last_name, encoding='utf8'))
    
    if (not name) or (not last_name):
        messagebox.showerror('Error', '¡Los campos de nombre y apellido no pueden estar vacíos!', parent=g_tkRoot)
        return
    
    # Verificar si hay un Arduino conectado.
    if not g_arduinoObj.is_available():
        messagebox.showerror('Error', '¡Conecte un Arduino al sistema!', parent=g_tkRoot)
        return
    
    # Mostrar diálogo para abrir archivo PNG.
    png_path = filedialog.askopenfilename(parent=g_tkRoot, title='Seleccione una imagen PNG', initialdir=os.path.abspath(os.path.dirname(__file__)), filetypes=[('Imágenes PNG', '.png')])
    if not png_path:
        return
    ...
    try:
        ...
        messagebox.showerror('Error', '¡El archivo proporcionado no es una imagen PNG!', parent=g_tkRoot)
        return
    except:
        traceback.print_exc()
        messagebox.showerror('Error', 'No se pudo abrir la imagen PNG.', parent=g_tkRoot)
        return
    ...
    conv_thread = threading.Thread(target=imageTracingThreadFunc, args=[name, last_name, png_path, image], daemon=False)
    conv_thread.start()
...
def main():
    ...
    print('Anchura: %u px. Altura: %u px. DPI: %u. Escala: %.2f%%.' % (screen_width_px, screen_height_px, screen_dpi, SCALE * 100.0))
    
    # Conectar a la base de datos MySQL.
    try:
        g_dbConnection = mysql.connector.connect(host=DB_HOST, user=DB_USER, password=DB_PASS, database=DB_NAME)
        g_dbCursor = g_dbConnection.cursor()
    except:
        traceback.print_exc()
        g_tkRoot.withdraw()
        messagebox.showerror('Error', "¡No se pudo conectar a la base de datos MySQL \"{}\" en \"{}\"!".format(DB_NAME, DB_HOST), parent=g_tkRoot)
        g_tkRoot.destroy()
        return
    ...
    g_tkRoot.protocol('WM_DELETE_WINDOW', uiHandleExitProtocol) # Interceptar el protocolo de salida de la ventana.
    ...
    g_tkCanvas.create_text(uiScaleMeasure(50), uiScaleMeasure(30), text='Nombre:', anchor=tk.CENTER)
    
    g_tkNameText = tk.Text(g_tkRoot, height=1, width=40, font=font.nametofont('TkDefaultFont'), wrap='none')
    ...
    g_tkCanvas.create_window(uiScaleMeasure(230), uiScaleMeasure(30), window=g_tkNameText, anchor=tk.CENTER)
    
    g_tkCanvas.create_text(uiScaleMeasure(50), uiScaleMeasure(60), text='Apellido:', anchor=tk.CENTER)
    
    g_tkLastNameText = tk.Text(g_tkRoot, height=1, width=40, font=font.nametofont('TkDefaultFont'), wrap='none')
    ...
    g_tkCanvas.create_window(uiScaleMeasure(230), uiScaleMeasure(60), window=g_tkLastNameText, anchor=tk.CENTER)
    
    g_tkOpenPngButton = tk.Button(text='Abrir PNG', command=uiOpenPng, width=10)
    g_tkCanvas.create_window(uiScaleMeasure(200), uiScaleMeasure(100), window=g_tkOpenPngButton, anchor=tk.CENTER)
    
    g_tkProgressText = g_tkCanvas.create_text(uiScaleMeasure(200), uiScaleMeasure(140), text='Conversión en progreso. Por favor, espere.', anchor=tk.CENTER)
    g_tkCanvas.itemconfigure(g_tkProgressText, state='hidden')
